...
import (
	...
	"github.com/murilocht/imersao17/goapi/internal/entity"
)
...
func (pd *ProductDB) GetProduct(id string) (*entity.Product, error) {
	var product entity.Product
	err := pd.db.QueryRow("SELECT id, name, price, category_id, image_url, description FROM products WHERE id = ?", id).Scan(&product.ID, &product.Name, &product.Price, &product.CategoryID, &product.ImageURL, &product.Description)
	if err != nil {
		return nil, err
	}
	return &product, nil
...
func (pd *ProductDB) CreateProduct(product *entity.Product) (string, error) {
	_, err := pd.db.Exec("INSERT INTO products (id, name, price, category_id, image_url, description) VALUES (?, ?, ?, ?, ?, ?)", product.ID, product.Name, product.Price, product.CategoryID, product.ImageURL, product.Description)
